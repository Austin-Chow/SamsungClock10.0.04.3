...
public class BottomNavigationView extends FrameLayout {
    ...
    static class SavedState extends AbsSavedState {
        ...
        static class C00301 implements ClassLoaderCreator<SavedState> {
            C00301() {
            }
    ...
    public BottomNavigationView(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        TintTypedArray a = TintTypedArray.obtainStyledAttributes(context, attrs, C0011R.styleable.BottomNavigationView, defStyleAttr, C0011R.style.Widget_Design_BottomNavigationView);
        if (a.hasValue(C0011R.styleable.BottomNavigationView_itemIconTint)) {
            this.mMenuView.setIconTintList(a.getColorStateList(C0011R.styleable.BottomNavigationView_itemIconTint));
        } else {
            this.mMenuView.setIconTintList(createDefaultColorStateList(16842808));
        }
        if (a.hasValue(C0011R.styleable.BottomNavigationView_itemTextColor)) {
            this.mMenuView.setItemTextColor(a.getColorStateList(C0011R.styleable.BottomNavigationView_itemTextColor));
        } else {
            this.mMenuView.setItemTextColor(createDefaultColorStateList(16842808));
        }
        if (a.hasValue(C0011R.styleable.BottomNavigationView_elevation)) {
            ViewCompat.setElevation(this, (float) a.getDimensionPixelSize(C0011R.styleable.BottomNavigationView_elevation, 0));
        }
        this.mMenuView.setItemBackgroundRes(a.getResourceId(C0011R.styleable.BottomNavigationView_itemBackground, 0));
        Drawable background = getBackground();
        if (background instanceof ColorDrawable) {
            this.mMenuView.setBackgroundColorDrawable((ColorDrawable) background);
        }
        if (a.hasValue(C0011R.styleable.BottomNavigationView_menu)) {
            inflateMenu(a.getResourceId(C0011R.styleable.BottomNavigationView_menu, 0));
        }
        a.recycle();
        ...
        this.mMenu.setCallback(new C00291());
    }
    ...
    private void addCompatibilityTopDivider(Context context) {
        ...
        divider.setBackgroundColor(ContextCompat.getColor(context, C0011R.color.sesl_bottom_navigation_shadow_color));
        divider.setLayoutParams(new LayoutParams(-1, getResources().getDimensionPixelSize(C0011R.dimen.sesl_bottom_navigation_shadow_height)));
        addView(divider);
    }
    ...
    private ColorStateList createDefaultColorStateList(int baseColorThemeAttr) {
        ...
        if (!getContext().getTheme().resolveAttribute(C0247R.attr.colorPrimary, value, true)) {
            return null;
        }
