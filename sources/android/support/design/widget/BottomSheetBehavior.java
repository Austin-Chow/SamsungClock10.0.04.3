...
public class BottomSheetBehavior<V extends View> extends Behavior<V> {
    ...
    private final Callback mDragCallback = new C00312();
    boolean mHideable;
    ...
    class C00312 extends Callback {
        ...
        public boolean tryCaptureView(View child, int pointerId) {
            ...
            if (BottomSheetBehavior.this.mState == 3 && BottomSheetBehavior.this.mActivePointerId == pointerId) {
                View scroll = BottomSheetBehavior.this.mNestedScrollingChildRef != null ? (View) BottomSheetBehavior.this.mNestedScrollingChildRef.get() : null;
                if (scroll != null && scroll.canScrollVertically(-1)) {
                    return false;
                }
            }
            boolean z = BottomSheetBehavior.this.mViewRef != null && BottomSheetBehavior.this.mViewRef.get() == child;
            return z;
        }
        ...
        public void onViewReleased(View releasedChild, float xvel, float yvel) {
            ...
            if (BottomSheetBehavior.this.mViewDragHelper == null || !BottomSheetBehavior.this.mViewDragHelper.settleCapturedViewAt(releasedChild.getLeft(), top)) {
    ...
    public BottomSheetBehavior(Context context, AttributeSet attrs) {
        super(context, attrs);
        TypedArray a = context.obtainStyledAttributes(attrs, C0011R.styleable.BottomSheetBehavior_Layout);
        TypedValue value = a.peekValue(C0011R.styleable.BottomSheetBehavior_Layout_behavior_peekHeight);
        if (value == null || value.data != -1) {
            setPeekHeight(a.getDimensionPixelSize(C0011R.styleable.BottomSheetBehavior_Layout_behavior_peekHeight, -1));
        } else {
            setPeekHeight(value.data);
        }
        setHideable(a.getBoolean(C0011R.styleable.BottomSheetBehavior_Layout_behavior_hideable, false));
        setSkipCollapsed(a.getBoolean(C0011R.styleable.BottomSheetBehavior_Layout_behavior_skipCollapsed, false));
        a.recycle();
    ...
    public boolean onLayoutChild(CoordinatorLayout parent, V child, int layoutDirection) {
        ...
        if (this.mPeekHeightAuto) {
            if (this.mPeekHeightMin == 0) {
                this.mPeekHeightMin = parent.getResources().getDimensionPixelSize(C0011R.dimen.design_bottom_sheet_peek_height_min);
            }
        ...
        if (child.isShown()) {
            ...
            switch (action) {
                case 0:
                    ...
                    if (this.mActivePointerId != -1 || parent.isPointInChildBounds(child, initialX, this.mInitialY)) {
                        z2 = false;
                    } else {
                        z2 = true;
                    }
            ...
            if (action != 2 || scroll == null || this.mViewDragHelper == null || this.mIgnoreEvents || this.mState == 1 || parent.isPointInChildBounds(scroll, (int) event.getX(), (int) event.getY()) || Math.abs(((float) this.mInitialY) - event.getY()) <= ((float) this.mViewDragHelper.getTouchSlop())) {
                z = false;
            }
            return z;
        }
    ...
    public boolean onTouchEvent(CoordinatorLayout parent, V child, MotionEvent event) {
        ...
        this.mVelocityTracker.addMovement(event);
        if (action == 2 && !this.mIgnoreEvents && this.mViewDragHelper != null && Math.abs(((float) this.mInitialY) - event.getY()) > ((float) this.mViewDragHelper.getTouchSlop())) {
    ...
    public void onNestedPreScroll(CoordinatorLayout coordinatorLayout, V child, View target, int dx, int dy, int[] consumed) {
        if (target == (this.mNestedScrollingChildRef != null ? (View) this.mNestedScrollingChildRef.get() : null)) {
    ...
    public boolean onNestedPreFling(CoordinatorLayout coordinatorLayout, V child, View target, float velocityX, float velocityY) {
        ...
        if (this.mState != 3 || super.onNestedPreFling(coordinatorLayout, child, target, velocityX, velocityY)) {
            return true;
        }
        return false;
    }
    ...
    void setStateInternal(int state) {
        if (this.mState != state) {
            this.mState = state;
            View bottomSheet = this.mViewRef != null ? (View) this.mViewRef.get() : null;
