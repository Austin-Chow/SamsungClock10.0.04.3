...
class SeslGradientColorSeekBar extends SeekBar {
    ...
    private int[] mColors = new int[]{ViewCompat.MEASURED_STATE_MASK, -1};
    ...
    private void initColor(int color) {
        ...
        float value = hsv[2];
        hsv[2] = 1.0f;
        this.mColors[1] = Color.HSVToColor(hsv);
        setProgress(Math.round(((float) getMax()) * value));
    }
    ...
    private void initProgressDrawable() {
        this.mProgressDrawable = (GradientDrawable) this.mContext.getDrawable(C0247R.drawable.sesl_color_picker_gradient_seekbar);
        setProgressDrawable(this.mProgressDrawable);
    }
