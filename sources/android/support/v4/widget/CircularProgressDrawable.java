...
public class CircularProgressDrawable extends Drawable implements Animatable {
    ...
    static final Interpolator SINE_IN_80_INTERPOLATOR = new PathInterpolator(MAX_PROGRESS_ARC, 0.0f, 0.83f, 0.83f);
    static final Interpolator SINE_IN_OUT_33_INTERPOLATOR = new PathInterpolator(0.33f, 0.0f, 0.67f, 1.0f);
    static final Interpolator SINE_IN_OUT_80_INTERPOLATOR = new PathInterpolator(0.33f, 0.0f, ARROW_SCALE_DURATION_OFFSET, 1.0f);
    static final Interpolator SINE_OUT_60_INTERPOLATOR = new PathInterpolator(0.17f, 0.17f, SHRINK_OFFSET, 1.0f);
    ...
    private void applyFinishTranslation(float interpolatedTime, Ring ring) {
        ...
        if (interpolatedTime <= ARROW_SCALE_DURATION_OFFSET) {
            ring.setArrowScale(1.0f - SINE_IN_OUT_33_INTERPOLATOR.getInterpolation(interpolatedTime / ARROW_SCALE_DURATION_OFFSET));
        }
        float targetRotation = (float) (Math.floor((double) (ring.getStartingRotation() / MAX_PROGRESS_ARC)) + 1.0d);
        ring.setStartTrim(ring.getStartingStartTrim() + (((ring.getStartingEndTrim() - MIN_PROGRESS_ARC) - ring.getStartingStartTrim()) * SINE_OUT_60_INTERPOLATOR.getInterpolation(interpolatedTime)));
        ring.setEndTrim(ring.getStartingEndTrim());
        ring.setRotation(ring.getStartingRotation() + ((targetRotation - ring.getStartingRotation()) * SINE_OUT_60_INTERPOLATOR.getInterpolation(interpolatedTime)));
        if (SINE_OUT_60_INTERPOLATOR.getInterpolation(interpolatedTime) > 0.7f && this.mAnimationEndCallback != null && !this.mEnding) {
    ...
    private void applyTransformation(float interpolatedTime, Ring ring, boolean lastFrame) {
        if (this.mFinishing) {
            ...
            if (interpolatedTime <= SHRINK_OFFSET) {
                ...
                startingStartTrim = ring.getStartingStartTrim() + (SINE_IN_OUT_80_INTERPOLATOR.getInterpolation(scaledTime) * 0.79f);
                startingStartTrim = ring.getStartingStartTrim();
                endTrim = startingStartTrim + ((SINE_IN_OUT_80_INTERPOLATOR.getInterpolation(scaledTime) * 0.79f) + MIN_PROGRESS_ARC);
            } else {
                scaledTime = (interpolatedTime - SHRINK_OFFSET) / 0.6f;
                endTrim = ring.getStartingEndTrim() + (SINE_IN_OUT_33_INTERPOLATOR.getInterpolation(scaledTime) * MIN_PROGRESS_ARC);
                endTrim = ring.getStartingStartTrim() + 0.79f;
                startingStartTrim = endTrim - (((1.0f - SINE_IN_OUT_33_INTERPOLATOR.getInterpolation(scaledTime)) * 0.79f) + MIN_PROGRESS_ARC);
            }
            float rotation = startingRotation + (0.25f * SINE_IN_OUT_33_INTERPOLATOR.getInterpolation(interpolatedTime));
