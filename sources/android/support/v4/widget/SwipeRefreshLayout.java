...
import android.support.v4.widget.CircularProgressDrawable.OnAnimationEndCallback;
...
public class SwipeRefreshLayout extends ViewGroup implements NestedScrollingChild, NestedScrollingParent {
    ...
    class C02072 implements OnAnimationEndCallback {
        C02072() {
        }
    ...
    class C02094 extends Animation {
        C02094() {
        }
    ...
    public void setRefreshOnce(boolean once) {
        if (once) {
            this.mProgress.setOnAnimationEndCallback(new C02072());
        } else {
    ...
    private void startScaleUpAnimation(AnimationListener listener) {
        ...
        this.mScaleAnimation = new C02083();
    ...
    void startScaleDownAnimation(AnimationListener listener) {
        ...
        this.mScaleDownAnimation = new C02094();
    ...
    public void setProgressBackgroundLightTheme(boolean isLight) {
        if (isLight) {
            this.mCircleView.setBackgroundColor(CIRCLE_BG_LIGHT);
        } else {
            this.mCircleView.setBackgroundColor(CIRCLE_BG_DARK);
        }
    }
    ...
    public boolean onInterceptTouchEvent(MotionEvent ev) {
        ...
        if (this.mActionDown && (action == 1 || (action == 2 && canChildScrollUp()))) {
    ...
    public void onNestedPreScroll(View target, int dx, int dy, int[] consumed) {
        if (dy > 0 && this.mTotalUnconsumed > 0.0f && this.mActionDown) {
    ...
    public void onNestedScroll(View target, int dxConsumed, int dyConsumed, int dxUnconsumed, int dyUnconsumed) {
        ...
        if (dy < 0 && !canChildScrollUp() && this.mActionDown) {
    ...
    private void moveSpinner(float overscrollTop) {
        ...
        float extraMove = (slingshotDist * (((float) (((double) (tensionSlingshotPercent / 4.0f)) - Math.pow((double) (tensionSlingshotPercent / 4.0f), 2.0d))) * DECELERATE_INTERPOLATION_FACTOR)) * DECELERATE_INTERPOLATION_FACTOR;
        int targetY = this.mSpinnerOffsetEnd;
        ...
        if (overscrollTop >= this.mTotalDragDistance && this.mProgress.getAlpha() < 255) {
            if (!isAnimationRunning(this.mAlphaMaxAnimation)) {
                startProgressAlphaMaxAnimation();
            }
        }
        this.mProgress.setStartEndTrim(-0.25f, Math.min(MAX_PROGRESS_ANGLE, dragPercent * MAX_PROGRESS_ANGLE) - 0.25f);
        this.mProgress.setArrowScale(Math.min(1.0f, dragPercent));
        this.mProgress.setAlpha((int) (255.0f * dragPercent));
        this.mProgress.setProgressRotation(dragPercent * 1.75f);
