...
public class ViewPager extends ViewGroup {
    ...
    private static final float SESL_DEFAULT_TOUCH_SLOP_RATE = 0.5f;
    ...
    private float mTouchSlopRatio = SESL_DEFAULT_TOUCH_SLOP_RATE;
    ...
    class C01854 implements OnApplyWindowInsetsListener {
        private final Rect mTempRect = new Rect();
    ...
    public static class SavedState extends AbsSavedState {
        ...
        static class C01861 implements ClassLoaderCreator<SavedState> {
            C01861() {
            }
    ...
    void initViewPager() {
        ...
        this.mSeslTouchSlop = configuration.getScaledTouchSlop();
        this.mSeslPagingTouchSlop = configuration.getScaledPagingTouchSlop();
        ...
        ViewCompat.setOnApplyWindowInsetsListener(this, new C01854());
    }
    ...
    public void setCurrentItemFromUser(int item, boolean fromUser) {
        boolean z;
        this.mPopulatePending = false;
        if (this.mFirstLayout) {
            z = false;
        ...
        setCurrentItemInternal(item, z, false, 0, fromUser);
    }
    ...
    void setCurrentItemInternal(int item, boolean smoothScroll, boolean always) {
        setCurrentItemInternal(item, smoothScroll, always, 0, false);
    }

    void setCurrentItemInternal(int item, boolean smoothScroll, boolean always, int velocity, boolean fromUser) {
    ...
    float distanceInfluenceForSnapDuration(float f) {
        return (float) Math.sin((double) ((f - SESL_DEFAULT_TOUCH_SLOP_RATE) * 0.47123894f));
    }
        ...
        switch (action) {
            case 0:
                ...
                if (this.mUsePagingTouchSlopForStylus) {
                    if (ev.isFromSource(InputDeviceCompat.SOURCE_STYLUS)) {
            ...
            case 2:
                ...
                if (activePointerId != -1) {
                    ...
                    if (dx == 0.0f || isGutterDrag(this.mLastMotionX, dx) || !canScroll(this, false, (int) dx, (int) x2, (int) y)) {
                        if (xDiff > ((float) this.mTouchSlop) && this.mTouchSlopRatio * xDiff > yDiff) {
                            this.mIsBeingDragged = true;
                            requestParentDisallowInterceptTouchEvent(true);
                            setScrollState(1);
        ...
        switch (ev.getAction() & 255) {
            ...
            case 1:
                if (this.mIsBeingDragged) {
                    ...
                    ItemInfo ii = infoForCurrentScrollPosition();
                    float marginOffset = ((float) this.mPageMargin) / ((float) width);
                    setCurrentItemInternal(determineTargetPage(ii.position, ((((float) scrollX) / ((float) width)) - ii.offset) / (ii.widthFactor + marginOffset), initialVelocity, (int) (ev.getX(ev.findPointerIndex(this.mActivePointerId)) - this.mInitialMotionX)), true, true, initialVelocity, true);
    ...
    public void endFakeDrag() {
        if (this.mFakeDragging) {
            if (this.mAdapter != null) {
                ...
                int scrollX = getScrollX();
                ItemInfo ii = infoForCurrentScrollPosition();
                setCurrentItemInternal(determineTargetPage(ii.position, ((((float) scrollX) / ((float) width)) - ii.offset) / ii.widthFactor, initialVelocity, (int) (this.mLastMotionX - this.mInitialMotionX)), true, true, initialVelocity, false);
            }
            endDrag();
    ...
    public boolean onGenericMotionEvent(MotionEvent event) {
        if (this.mIsMouseWheelEventSupport && (event.getSource() & 2) != 0) {
    ...
    public void setMouseWheelEventSupport(boolean support) {
        this.mIsMouseWheelEventSupport = support;
    }

    public void seslSetConfigurationChanged(boolean changed) {
    ...
    public void seslSetTouchSlop(int slopConstant) {
        this.mSeslTouchSlop = slopConstant;
    }

    public int seslGetTouchSlop() {
        return this.mSeslTouchSlop;
    }

    public void seslSetTouchSlopRatio(float ratio) {
        this.mTouchSlopRatio = ratio;
    }
    ...
    public float seslGetDefaultTouchSlopRatio() {
        return SESL_DEFAULT_TOUCH_SLOP_RATE;
    }
}
